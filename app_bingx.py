...
def get_data(symbol="BTC/USDT", interval="15m", limit=200):
    ...
    if not data or len(data) < 50:
        st.error(f"Pas de données reçues: {data}")
        return None
...
symbol = st.sidebar.selectbox("Paire", ["BTC/USDT","ETH/USDT","SOL/USDT","BNB/USDT","BTCUSDT"])
...
if st.sidebar.button("🔍 Analyser"):
    with st.spinner("Chargement..."):
        ...
        if df is not None:
            ...
            if len(df) == 0:
                st.error("Toujours pas de données après calcul RSI")
            else:
                ...
                if last_rsi < 30:
                    ...
                    st.error(f"❤️ VENTE - RSI {last_rsi:.2f}")
                else:
